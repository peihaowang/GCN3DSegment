...
def serialize_example(mesh, labels):
    ...
    feature = {
        ...
        , 'vertices': _bytes_feature(tf.io.serialize_tensor(vertices))
        , 'triangles': _bytes_feature(tf.io.serialize_tensor(triangles))
...
def serialize_mesh(writer, mesh_path, label_path):
    if os.path.exists(mesh_path) or os.path.exists(label_path):
        return False
...
if __name__ == "__main__":
    ...
    with tf.python_io.TFRecordWriter(g_output_path) as writer:
